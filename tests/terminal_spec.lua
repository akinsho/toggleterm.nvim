...
describe("ToggleTerm tests:", function()
  ...
  describe("toggling terminals - ", function()
    ...
    it("should toggle a terminal if not hidden", function()
    ...
    it("should open a hidden terminal and a visible one", function()
      ...
      hidden:toggle()
      visible:toggle()
      assert.is_true(ui.term_has_open_win(hidden))
      assert.is_true(ui.term_has_open_win(visible))
    end)
  ...
  describe("context-toggling terminals - ", function()
    before_each(
      function()
        toggleterm.setup({
          get_ctx = function ()
    ...
    it("should associate terminal with current context", function()
      local term = Terminal:new():toggle()
      assert.are_equal(t.get_ctx_term(), term)
    end)
    it("should not associate hidden terminals", function()
      Terminal:new({ hidden = true }):toggle()
      assert.is_falsy(t.get_ctx_term())
    end)
    it("should not override context", function()
      local term1 = Terminal:new():toggle()
      assert.are_equal(t.get_ctx_term(), term1)
      assert.is_true(ui.term_has_open_win(term1))
      -- testing this command (no-arg `:ToggleTerm`) as well
      toggleterm.toggle(0)
      -- no-arg `:ToggleTerm` should close any open terminals
      assert.is_false(ui.term_has_open_win(term1))
      Terminal:new():toggle()
      assert.are_equal(t.get_ctx_term(), term1)
    end)
    it("should handle multiple terminals with different contexts", function()
      local term1 = Terminal:new():toggle()
      assert.are_equal(t.get_ctx_term(), term1)
      term1:toggle()
      vim.cmd("tabnew")
      local term2 = Terminal:new():toggle()
      assert.are_equal(t.get_ctx_term(), term2)
      assert.is_true(ui.term_has_open_win(term2))
      term2:toggle()
      vim.cmd("tabprevious")
      assert.are_equal(t.get_ctx_term(), term1)
      -- no-arg `:ToggleTerm` should open based on context
      toggleterm.toggle(0)
      assert.is_true(ui.term_has_open_win(term1))
      toggleterm.toggle(0)
      vim.cmd("tabnext")
    ...
    it("should be able to clear context", function()
      local term1 = Terminal:new():toggle()
      assert.are_equal(t.get_ctx_term(), term1)
      term1:toggle()
      vim.cmd("tabnew")
      ...
      assert.are_equal(t.get_ctx_term(), term1)
      t.clear_ctx_term()
      local term3 = Terminal:new():toggle()
      assert.are_equal(t.get_ctx_term(), term3)
    end)
    it("no-arg :ToggleTerm should use last-used terminal when one has not been set for this context", function()
      local term1 = Terminal:new():toggle()
      assert.are_equal(t.get_ctx_term(), term1)
      term1:toggle()
      vim.cmd("tabnew")
